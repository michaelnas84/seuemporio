...
<table class="table table-sm mt-3 tabelas">
	...
	if($txtbuscar == ''){
		$res = $pdo->query("SELECT * from vendas order by data DESC LIMIT $limite, $itens_por_pagina");
	}else{
		$txtbuscar = '%'.@$_POST['txtbuscar'].'%';
		$res = $pdo->query("SELECT * from vendas where cliente LIKE '$txtbuscar' or id LIKE '$txtbuscar' order by data asc");
	...
	for ($i=0; $i < count($dados); $i++) { 
			...
			$total 				= $dados[$i]['total'];
			$total_dinheiro 	= $dados[$i]['total_pago'];
			$troco 				= $dados[$i]['troco'];
...
echo '
		<tr>
			...
			<td>'.date('d/m/Y',  strtotime($data)).' - '.$hora.'</td>
